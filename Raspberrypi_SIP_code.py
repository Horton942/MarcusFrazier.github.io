...
class user_app_callback_class(app_callback_class):
    def __init__(self):
        ...
        self.servo = AngularServo(18, min_pulse_width=0.0005, max_pulse_width=0.0024)
        self.ultrasonic = DistanceSensor(echo=17, trigger=4, max_distance=1.219)
...
def app_callback(pad, info, user_data):
    ...
    if object_detected:
        ...
        if user_data.detection_counter >= 4 and not user_data.is_it_active and user_data.ultrasonic.wait_for_in_range():
            ...
            user_data.ultrasonic.wait_for_in_range = True
            print("In range")
    else:
